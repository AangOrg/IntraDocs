...
8. **Fixture pengguna nonaktif sudah ada di mockup.** Layar 8 mencantumkan Fajar Nugroho sebagai Viewer dari Finance (mitra) dengan cakupan "SOP & Proses (baca)" dan status **Nonaktif** — lengkap dengan "Terakhir Aktif: 3 minggu lalu". Tabel pengguna seed `docs/rbac-matrix.md` justru menempatkan Fajar Nugroho sebagai viewer aktif bercakupan semua kategori, lalu menambahkan pengguna keenam bernama karangan untuk memenuhi `tests/rbac/inactive-user.spec.ts`. Fixture itu seharusnya Fajar Nugroho sendiri. Perubahannya menyentuh tabel seed, T-003, dan T-008, jadi diserahkan ke pemilik repo dan belum dikerjakan di sini.
